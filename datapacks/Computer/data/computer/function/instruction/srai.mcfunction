tellraw @a[tag=DEBUG] [{"text":""},{"text":""},{"text":"[DEBUG] - ","bold":true,"color":"blue"},{"text":"Running srai","color":"gold"}]
# tellraw @a[tag=ERROR] [{"text":""},{"text":""},{"text":"Error: Not Yet Implemented srai","bold":true,"color":"red"}]
...
execute if score keep_going Computer matches 1 run function computer:alu/shift_arith_rd_right_based_on_rs2_5bits

execute if score count Computer matches 32.. run tellraw @a[tag=ERROR] [{"text":"Error: srai, recursive function as been called more than 31 times","color": "red","bold": true}]
