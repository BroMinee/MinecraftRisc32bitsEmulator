...
if [ "$EXTENSION" == "c" ]; then
    ...
    $COMPILER -march=rv32im -O3 -mabi=ilp32 -c "$SOURCE_FILE" -o "$OBJ_FILE"
    if [ $? -ne 0 ]; then
        echo "Error: Compilation of C file failed."
        exit 2
    fi
    $COMPILER -march=rv32im -O3 -mabi=ilp32 -S "$SOURCE_FILE"
    if [ $? -ne 0 ]; then
...
hexdump -v -C "$BIN_FILE" > /datapacks/output_dump.txt
echo "Hexdump saved to /datapacks/output_dump.txt"
echo "Done!"
